...
def generate_months_passed_since_first_sub(path_in,path_out,file_in,last_book_file_in,first_sub_file_in,file_out):
    #---------------------#
    #- Reading stg files -# To refactor
    #---------------------#

    stg_subscription_df   = pd.read_csv(path_in+file_in          , parse_dates=['sub_date'])
    last_booking_df       = pd.read_csv(path_in+last_book_file_in, parse_dates=['last_booking_date'])
    first_subscription_df = pd.read_csv(path_in+first_sub_file_in, parse_dates=['first_subscription_date'])
    ...
    months_since_first_sub_df = last_booking_df\
                                            .merge(\
                                                  first_subscription_df
                                                , on='subscriber_id'
                                                , how='left'
                                            )

    ops.date_diff_months(  months_since_first_sub_df
    ...
    months_since_first_sub_df = stg_subscription_df\
                                                .merge(\
                                                      first_subscription_df
                                                    , on='subscriber_id'
                                                    , how='left'
