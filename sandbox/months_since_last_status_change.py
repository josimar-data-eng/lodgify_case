...
def generate_months_status_change(path_in,path_out,file_in,file_out):
    ...
    months_status_change_df = stg_subscription_df\
                                            .filter(
                                                    [ 'sub_id'
                                                    , 'status'
                                                    , 'sub_year_month'
    ...
    months_status_change_df.sort_values(by = ['sub_id', 'sub_year_month'], ascending = [True, True],inplace=True)

    ops.positional_function('lag' 
    ...
    for index, row in months_status_change_df.iterrows():        
        if row['previous_status'] != '':
            ...
            months_status_change_df.at[index,'idc_status_change'] = 0
    months_status_change_df['idc_status_change'] = months_status_change_df['idc_status_change'].astype(int)

    only_status_change_df = months_status_change_df.query("idc_status_change == 1")
    
    first_status_change_df = ops.groupby( only_status_change_df
                                        , ['sub_id']
                                        , 'sub_date'
                                        , 'min'
                                        , 'first_status_change'
    ...
    for index, row in months_status_change_df.iterrows():
        if (row['previous_idc_status_change']) == 0 and row['first_status_change'] != True:
            months_status_change_df.loc[index,'last_change'] = months_status_change_df.loc[index-1,'last_change']
    ...
    ops.date_diff_months(months_status_change_df,'last_change','sub_date',0,'qtt_months',True)

    months_status_change_df = months_status_change_df\
                                                .filter(
                                                        ['sub_id'
                                                        ,'status'
                                                        ,'sub_year_month'
