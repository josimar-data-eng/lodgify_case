...
def get_months_per_status(path_in, path_out, file_in, file_out):
    # ------------------------------------------------------------#
    ...
    count_status_df = stg_subscription_df.filter(["sub_id", "status", "sub_year_month"])
    ...
    count_status_df.sort_values(
        by=["sub_id", "sub_year_month"], ascending=[True, True], inplace=True
    )

    count_status_df["qtt_monhts_active"] = (
        count_status_df.groupby(["sub_id"])["is_active"].cumsum().astype(int)
    )

    count_status_df["qty_monhts_canceled"] = (
        count_status_df.groupby(["sub_id"])["is_canceled"].cumsum().astype(int)
    )

    count_status_df = count_status_df.filter(
        ["sub_id", "sub_year_month", "qtt_monhts_active", "qty_monhts_canceled"]
    )
    count_status_df.to_csv(path_out + file_out)
    print(
        f"File {file_out} loaded at {path_out} directory with {len(count_status_df.index)} rows."
    )
